...
**Mail at rest is encrypted — but not end-to-end.** Maildirs are never stored plaintext on disk: full-disk `softraid` encryption protects a stolen/pulled disk, and the off-box backup is encrypted at rest (§5.5). "Encrypted at rest" means a disk or backup walking away can't be read; it does *not* mean the admin can't read mail, since Dovecot holds the decryption key to serve IMAP. This distinction is stated plainly in user-facing materials — the platform's trust claim is verifiability ("audit us"), not zero-knowledge mail.
...
Mail storage is encrypted at rest (§2): Maildirs are never stored plaintext on disk. Resolved: **full-disk `softraid` crypto** (protects a stolen/pulled disk, near-zero cost on AES-NI hardware) **plus an at-rest-encrypted off-box backup** (§5.5). `mail_crypt` was considered and **dropped**: with the backup already encrypted, its only marginal protection — keeping the Maildir non-plaintext on the backup copy — is already covered, and it buys nothing for the accepted costs of degraded server-side IMAP SEARCH and per-user keypair management. Honest limit, stated plainly: none of this stops the admin from reading mail, since Dovecot holds the decryption key at runtime (§2).
...
2. Get OpenSMTPD + Dovecot + Maildir working for Oliver's own email, which pulls in DNS from day one — `nsd` (hidden primary) + Hurricane Electric secondary + MX/SPF/DKIM/DMARC (§5.7) — dogfooding surfaces deliverability and config problems before any other user is exposed to them
...
17. ~~Mail-at-rest encryption mechanism (§2, §5.1)...~~ — resolved: **full-disk `softraid` + at-rest-encrypted backup**, `mail_crypt` dropped as redundant (§2, §5.1).
