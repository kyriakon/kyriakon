...
**Cost model corrected (2026-09-22).** The paragraph above records three fixed lines, and the deployment has five. The restore and rehearsal box is a second Hetzner instance, planned in §5.5 as a throwaway machine but run continuously since it carries the weekly restore test, so it is a standing line rather than a per-rehearsal cost. The second domain was not counted at all: `kyriakon.com` is a defensive registration that renews every year (ADR 0001), and both names now sit at Porkbun. The three recorded lines stand, so the fixed total is the recorded ~£113 plus a second small instance of the primary's family plus a second domain renewal at Porkbun list price (`.com` at 11.08 USD registration and renewal, checked 2026-09-22), which puts it at roughly £160 to £190 and break-even at about 8 to 10 paying users rather than ≈ 6. The restore instance is the one figure to read off the current invoice before quoting a precise number, since Hetzner renders prices client-side. The £20/yr price and the method are unchanged, and the same correction is what ADR 0006's revenue gate rests on.
